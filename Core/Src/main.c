...
/* USER CODE BEGIN PFP */
void UART2_Init_Receive_IT(void);
...
void UART2_Init_Receive_IT(void) {
    ...
    HAL_UART_Receive_IT(&huart2, (uint8_t*)&rx_byte, 1); // Receive the firest byte
}
...
int main(void) {
    ...
    UART2_Init_Receive_IT(); // Start UART reception in interrupt mode
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim) {
    ...
    if (htim->Instance == TIM4) {
        // PID_Loop(); // Call the PID loop function
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
    if (huart->Instance == USART2) {
        UART2_Receive_Handler(rx_byte);                      // Handle the received byte
        HAL_UART_Receive_IT(&huart2, (uint8_t*)&rx_byte, 1); // Continue receiving data
    }
}
/* USER CODE END 4 */
